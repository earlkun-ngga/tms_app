...
class FormInputMaintenance extends StatefulWidget {
  String plateNo;
  String idCar;
  String idDriver;
  String userId;
  ...
  @override
  _FormInputMaintenanceState createState() => _FormInputMaintenanceState();

  FormInputMaintenance(this.plateNo, this.idDriver, this.idCar, this.userId, this.jumlahTire);
}

class _FormInputMaintenanceState extends State<FormInputMaintenance> {
  ...
  void initState() {
    ...
    print('step2FormInputServiceCar.dart : ID Car -> ${widget.idCar}');
            ...
            child: Row(
              ...
              children: <Widget>[
                Column(
                  ...
                  Text('Plate No Car : ${widget.plateNo} ', style: TextStyle(
                    color: quiz_textColorSecondary, fontSize: 20.0
                  ),),
                  SizedBox(height: 10.0),
                  // Text('Service ID : #1000111011', style: TextStyle(
                  //   color: quiz_textColorSecondary, fontSize: 20.0
                  // ),),
                ...
                SingleChildScrollView(
                  ...
                  child: Container(
                    ...
                    child: selectedPos == 1 ? ServiceCarForm('${widget.idDriver}', '${widget.idCar}', '${widget.userId}') : selectedPos == 2 ? TireFormAir('${widget.jumlahTire}', '${widget.idCar}','${widget.userId}', '${widget.idDriver}') : TireFormChange('${widget.jumlahTire}', '${widget.idCar}','${widget.userId}', '${widget.idDriver}'),
                  ),
                ),
              ],
            ),
...
class ServiceCarForm extends StatefulWidget {
  ...
  @override 
  _ServiceCarFormState createState() => _ServiceCarFormState();
  ServiceCarForm(this.idDriver, this.idCar, this.userId);

}


class _ServiceCarFormState extends State<ServiceCarForm> {
  ...
  void initState() {
    super.initState();
    servmod2.getServiceItem().then((value) => {
            ...
            children: [
              CardSettingsSection(
                children: [
                    ...
                    height: 50,
                    child: Center(
                      child: Text('FORM SERVICES', style: TextStyle(fontSize: 20)),
              ),
            ),
              ),
                 ...
                 CardSettingsButton(
                   ...
                   onPressed: (){
                    if(this.selectedService == '[]' || this.selectedService == ''){
                     ...
                     } else if(this.kilometerText.text == '')
                     {
                        ...
                        showDialog(
                                ...
                                builder: (BuildContext context) => new AlertDialog(
                                  title: new Text('Peringatan'),
                                  content: new Text('Pastikan Mengisi Kilometer Terakhir !'),
                                  actions: <Widget>[
                                    new IconButton(
                                        icon: new Icon(Icons.close),
                                        onPressed: () {
                                ...
                                builder: (BuildContext context) => new AlertDialog(
                                  ...
                                  actions: <Widget>[
                                    Row(
                                      ...
                                      final responseApi = await servmod2.setMaintenanceCar(
...
class _TireFormAirState extends State<TireFormAir> {
  ...
  initState(){
    print('jumlah tire nya : ${this.jumlahTire}');
     ...
     carmod.getInfoTireCar('${widget.idCar}').then((value) => {
  ...
  Widget build(BuildContext context)
  {
    return FutureBuilder(
      ...
      future: carmod.getInfoTireCar('${widget.idCar}'),
      builder: (BuildContext context, AsyncSnapshot snapshot){
                                ...
                                builder: (BuildContext context) => new AlertDialog(
                                  ...
                                  actions: <Widget>[
                                    Row(
                                      ...
                                      print('User Id -> ${widget.userId}');
                                      buatLoadingLogin(context);
                                      final responseApi = await servmod2.setMaintenanceTireCarAir(
                                        "${this.idMaintenanceSf}", 
                                        "${widget.idDriver}", 
                                        "${widget.idCar}", 
                                        "${engineerText.text.toString()}", 
                                        "${kilometerText.text.toString()}",
                                        "${this.serviceDate}", 
                                        "${this.serviceTime}", 
                                        "${this.estimateFinishDate}", 
                                        "${this.estimateTime}",
          ...
          children: [
            CardSettingsButton(
              ...
              if(this.idLokasiBengkel == '')
                     ...
                     } else if(this.kilometerText == '')
                     {
                       showDialog(
                                context: context,
                                builder: (BuildContext context) => new AlertDialog(
                                title: new Text('Peringatan'),
                                content: new Text('Pastikan Kilometer di isi !'),
                                actions: <Widget>[
                                new IconButton(
                                icon: new Icon(Icons.close),
                                        onPressed: () {
                        ...
                        title: new Text('Validasi Data '),
                                  ...
                                  actions: <Widget>[
                                    Row(
                                      ...
                                      print('User Id -> ${widget.userId}');
                                      buatLoadingLogin(context);
                                      final responseApi = await servmod2.setMaintenanceTireCarChange(
                                        "${this.idMaintenanceSf}", 
                                        "${widget.idDriver}", 
                                        "${widget.idCar}", 
                                        "${engineerText.text.toString()}", 
                                        "${this.idLokasiBengkel}",
                                        "${this.kilometerText.text}",
                                        "${this.serviceDate}", 
                                        "${this.serviceTime}", 
                                        "${this.estimateFinishDate}", 
                                        "${this.estimateTime}",
